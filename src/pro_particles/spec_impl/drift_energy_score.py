...
def drift_energy_score(
    *,
    ...
    """Energy score drift using a user-supplied ∇_1 L_energy(θ, θ'; x).

    Note: The paper does not provide an explicit W(Q) formula for the energy score.
    This implementation mirrors the MMD^2 structure and requires grad_L_energy.
    """
    ...
    for j in range(p):
        ...
        for k in range(p):
            if leave_one_out and k == j:
                continue
            for i in range(n):
                interaction[i] += grad_L_energy(particles[j], particles[k], x_obs[i])
    ...
    if not np.all(np.isfinite(drift)):
        raise ValueError("drift_energy_score produced non-finite values.")
